...
break;}
return isDarkTheme;}
function renderThemeVariation(isDarkTheme,init=false){const codeHlEnabled=$('link[title=hl-light]').length>0;const codeHlLight=$('link[title=hl-light]')[0];const codeHlDark=$('link[title=hl-dark]')[0];const diagramEnabled=$('script[title=mermaid]').length>0;if(!init){if((isDarkTheme===0&&!$('body').hasClass('dark'))||(isDarkTheme===1&&$('body').hasClass('dark'))){return;}}
...
if(diagramEnabled){if(init){mermaid.initialize({theme:'default',securityLevel:'loose'});}else{location.reload();}}}else if(isDarkTheme===1){if(!init){$('body').css({opacity:0,visibility:'visible'}).animate({opacity:1},500);}
